<template>
  <div class="flex flex-row items-end justify-between self-stretch shrink-0 relative">
    <div class="flex flex-col gap-1">
      ...
      <h5 class="text-[#000000] text-left relative" style="font: 700 14px 'Product Sans', sans-serif">
        {{ acronym }}
      </h5>
    </div>

    <span class="text-[#363636] text-left relative" style="font: 700 14px 'Product Sans', sans-serif">
      {{ dataComputed }}
    </span>
  </div>
</template>
...
const percentaje = useDataEmpty.product.subtotal ? (useDataEmpty.product.subtotal * props.rate) / 100 : 0
