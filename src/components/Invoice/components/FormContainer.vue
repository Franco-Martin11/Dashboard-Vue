<template>
  <div v-motion-fade class="flex flex-col px-5 gap-[25px] items-start justify-start flex-[1_1_350px] relative">
    <!-- invoice title -->
    ...
    <form action="" @submit.prevent="emitirEvento" :v-model="dataForm" class="w-full flex flex-col gap-4">
      <!-- Languaje dropdownd -->
      ...
      <div
        ...
        <template v-for="formData in dataForm" :key="formData.category">
          <div class="flex flex-col gap-5 items-start justify-start self-stretch shrink-0 relative w-full">
            <h3 class="text-[#15161f] text-left relative" style="font: 400 18px 'Product Sans Medium', sans-serif">
              {{ formData.category }}
            </h3>
